...
Tanto en animales como en plantas, el interés principal en el mapeo de QTL consistió en un método conocido como selección asistida por marcadores (MAS) [@cite:3], proceso en el cual los individuos portadores de un marcador de ADN deseado podían ser identificados y utilizados para aumentar la respuesta genética de caracteres cuantitativos importantes en la agricultura [@cite:32]. @cite:3 describen la MAS (Figura 1.2) como un proceso de tres pasos, en el cual se detecta uno o varios QTL que posteriormente permiten encontrar el gen responsable de la variación genética que más adelante al ser seleccionado aumenta la frecuencia del alelo favorable.

```{r, echo = FALSE, out.width = "100%", fig.align = 'center', fig.showtext = TRUE}

Punto_1 <- data.frame(x = 4.5, y = 5.8)
Punto_2 <- data.frame(x = 5.0, y = 4.8)
Punto_3 <- data.frame(x = 5.3, y = 6.1)

data.frame(
  x = c(0.0:10.0), y = c(-1.0:9.0)
  ) %>%
  ggplot(aes(x, y)) +
  #geom_blank() +
  ...
  geom_point(aes(x = x, y = y), data = Punto_1, size = 11.6, shape = 22, color = 'black', fill = 'black', alpha = 0.4) +
  annotate(geom = 'text', x = 4.5, y = 5.8, label = 'QTL', size = 3.8, colour = 'black') +
  
  geom_point(aes(x = x, y = y), data = Punto_2, size = 11.6, shape = 25, color = 'black', fill = 'black', alpha = 0.4) +
  annotate(geom = 'text', x = 5.0, y = 4.8, label = 'QTL', angle = 60, size = 3.8, colour = 'black') +
  
  geom_point(aes(x = x, y = y), data = Punto_3, size = 11.6, shape = 21, color = 'black', fill = 'black', alpha = 0.4) +
  annotate(geom = 'text', x = 5.3, y = 6.1, label = 'QTL', size = 3.8, colour = 'black') +
  ...
  #annotate(geom = 'text', x = 5.0, y = 8.5, label = 'Identificación', size = 4.6, colour = 'black') +
  #annotate(geom = 'text', x = 5.0, y = 8.5, label = 'de QTL', size = 4.6, colour = 'black') +
  
  scale_x_continuous(limits = c(0.0, 10.0)) +
  scale_y_continuous(limits = c(-1.0, 9.0)) +
