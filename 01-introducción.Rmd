...
Al publicar su propuesta innovadora de selección genómica, @cite:8 dieron a conocer también la forma en como la misma podía resolverse. La idea consistió en representar la relación entre el fenotipo y el genotipo mediante un modelo de regresión lineal (Figura 1.4), que luego podía usarse para predecir el GEBV de los individuos [@cite:6; @cite:31].
...
ggplot(data = Gen_1_Gen_2, aes(x = x, y = y, colour = Genotipos, fill = Genotipos)) +
  ...
  annotate(geom = 'text', x = 1.50, y = 1.44, label = TeX('$y_{i} = \\mu 1_{n} + x_{i1}g_{1}$'), size = 4.8) +
...
::: {.center data-latex=""}
__Figura 1.4:__ . Figura adaptada de @cite:6.
:::

La expresión se presenta a continuación

\begin{equation}
y_{i} = \mu 1_{n} + \sum_{j = 1}^{p}x_{ij}g_{j}+e_{i},
\end{equation}
...
::: {.center data-latex=""}
__Figura 1.5:__ . Figura adaptada de @cite:31.
:::
