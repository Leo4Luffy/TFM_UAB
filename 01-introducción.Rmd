...
pacman::p_load(dplyr, ggplot2, showtext, patchwork, latex2exp)
...
::: {.center data-latex=""}
__Figura 1.5:__ . Figura adaptada de @cite:31 y de @cite:7.
:::

Cuando @cite:8 propusieron por primera vez la teoría de la selección genómica, proporcionaron también una serie de métodos para resolver el problema mencionado en el parrafo anterior. Uno de estos métodos es el mejor predictor lineal insesgado por regresión de crestas (rrBLUP), el cual se puede expresar como un modelo lineal mixto:

\begin{equation}
y = Xb + Zg + e,
\end{equation}

donde $y$, $Z$ y $e$ denotan los mismos términos del modelo (1.2), $g$ es un vector de efectos del marcador en el fenotipo (tratados aquí como efectos aleatorios), $X$ es una matriz que indica los efectos fijos y $b$ es un vector de efector fijos. Luego la predicción del GEBV se realiza a partir de $\hat{g} = (Z'Z + I \lambda)^{- 1} Z'y$, donde $I$ es una matriz identidad y $\lambda$ es un factor de penalización que se agrega a la diagonal de $Z'Z$, y permite estimar cualquier número de efectos de marcador. Dicho factor de penalización se estima por máxima verosimilitud restringida (REML) como $\frac{\sigma^{2}_{e}} {\sigma^{2}_{g}}$ <!-- es decir, la relación entre los dos componentes de varianza del efecto aleatorio -->, donde $\sigma^{2}_{g}$ es la varianza del efecto del marcador y $\sigma^{2}_{e}$ es la varianza del error residual. Con rrBLUP, se reducen todos los efectos hacia cero por igual, lo que corresponde al supuesto de que todos los marcadores tienen una varianza común y son normalmente distribuidos [@cite:7].

Otro de los métodos propuesto por @cite:8 corresponde los métodos Bayesianos.
