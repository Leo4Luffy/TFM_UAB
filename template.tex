...
Esta tesis se escribio usando los paquetes de R (R) Markdown, \LaTeX\ y \verb+bookdown+.
...
\mbox{ante una comisión constituida por la Junta de Máster,}\\
para ser defendido en publico el\\
$if(date)$$date$$else$[………]dag […date…] […month…] 20[..]$endif$, a las $if(time)$$time$$else$[…time…]$endif$  \\ }\par %
\vspace{\baselineskip}
{\Large $if(author)$$author$$else$[…given names in full…] [… surname…]$endif$}\par
\vspace{\baselineskip}
\includegraphics[width=80mm]{_bookdown_files/logo_crag.jpg} \newline
\end{center}
...
$if(one_promotor)$Promotor:$else$Director:$endif$
$for(promotores)$
