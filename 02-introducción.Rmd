...
pacman::p_load(dplyr, ggplot2, showtext, patchwork, latex2exp, gt, ggforce, here, fmsb, readr, tidyr, png)
...
La contribución en la genética cuantitativa y molecular de distintas personas, permitieron el desarrollo<!-- de lo que hoy en día se conoce como--> de la selección genómica (Figura 2.1).

<!--La historia de la genética cuantitativa y molecular se debe a la contribución de muchas personas (Figura 2.1), hecho que permitió la conexión entre ambas disciplinas y el desarrollo de lo que hoy en día se conoce como selección genómica.-->
...
data.frame(
  ...
  annotate(geom = 'segment', x = 2001, xend = 2001, y = -0.7, yend = 2.2, colour = 'red', linetype = 'dashed') +
  annotate(geom = 'rect', xmin = 1987, xmax = 2015, ymin = -0.7, ymax = -2.5, colour = 'red', fill = 'red', alpha = 0.2) +
  annotate(geom = 'text', x = 2001, y = -1.0, label = '2001', family = 'gochi', size = 3.4, colour = 'black') +
  annotate(geom = 'text', x = 2001, y = -1.4, label = 'Meuwissen, Hayes y', size = 2.2, colour = 'black') +
  annotate(geom = 'text', x = 2001, y = -1.8, label = 'Goddard proponen la', size = 2.2, colour = 'black') +
  annotate(geom = 'text', x = 2001, y = -2.2, label = 'selección genómica.', size = 2.2, colour = 'black') +
...
```
__Figura 2.2:__ Esquema de la MAS. En la MAS, los fenotipos y genotipos de la población de mapeo se analizan usando un modelo estadístico, identificando con ello relaciones significativas entre fenotipos y genotipos. Por último, se seleccionan los individuos favorables con base en datos de genotipo. Figura adaptada de @cite:6.
...
```
__Figura 2.3:__ Esquema de la selección genómica. La selección genómica utiliza un modelo estadístico, diseñado a partir de datos genotípicos y fenotípicos en una población de entrenamiento, para predecir el GEBV de los individuos en una población de prueba con datos genotípicos. Por último, los individuos se seleccionan de acuerdo a su GEBV. Figura adaptada de @cite:7. <!-- NOTA: la selección genómica difiere significativamente de la selección convencional asistida por marcadores, ya que permite seleccionar individuos sin conocimiento previo de una asociación entre los marcadores y el rasgo de interés. La selección genómica se refiere a métodos que utilizan marcadores de todo el genoma para predecir el valor genómico de un individuo con suficiente precisión para permitir la selección de individuos basándose únicamente en esa predicción. -->
...
En la selección genómica, la relación genotipo-fenotipo suele ser representada como un modelo lineal (Figura 2.4). Por tanto, el modelo de regresión lineal es un enfoque fundamental en la selección genómica [@cite:6; @cite:31; @cite:37].
...
```
__Figura 2.4:__ Relación genotipo-fenotipo de individuos (círculos amarillo y azul) <!--en una población de entrenamiento -->para un solo marcador. $Y_{i}$ y $x_{i1}$ denotan los fenotipos y genotipos, y $\mu$ y $g_{i}$ son los parámetros a determinar. Los genotipos bialélicos se codifican como 0 y 1, y los fenotipos se distribuyen de acuerdo a una normal. Figura adaptada de @cite:6.
...
```
__Figura 2.5:__ Enfoques estadísticos de la selección genómica. Cada uno de estos enfoques suponen distintas distribuciones de efectos de los marcadores sobre el carácter. Figura adaptada de @cite:31 y de @cite:7.

Al proponer la teoría de la selección genómica, @cite:8 proporcionaron también una serie de métodos estadísticos como solución al problema planteado en el párrafo anterior, esto es, el mejor predictor lineal insesgado por ridge-regression (rrBLUP), y los métodos Bayesianos BayesA y BayesB.
...
Los modelos de regresión Bayesianos (BayesA y BayesB) propuestos por @cite:8, se diferencian en los distintos supuestos sobre los efectos del marcador y sus distribuciones. De acuerdo a @cite:21, en BayesA se supone que los efectos de los marcadores se distribuyen de acuerdo a una distribución t de Student en lugar de una normal, permitiendo así que algunos marcadores tengan mayor efecto que otros. En cuanto a BayesB, éste presenta los mismos supuestos de BayesA, sin embargo, a diferencia de este último, en BayesB se permite que una parte de los marcadores no tengan efecto alguno sobre el carácter [@cite:21].
...
La idea de @cite:39, al sustituir la matriz A por la matriz G, consistió en precisar el parentesco real entre individuos al momento de estimar los valores de cría. Según @cite:21, el parentesco que proviene al usar la matriz A es un parentesco esperado, lo cual puede no reflejar el porcentaje real de genes idénticos entre dos individuos emparentados, mientras que G refleja mejor el parentesco realizado (Figura 2.6).
...
Para implementar los métodos de selección genómica mencionados anteriormente (rrBLUP,  BayesA-B-C, LASSO Bayesiano y GBLUP), es necesario disponer de información genotípica y fenotípica de todos los individuos. Esta situación puede ser desventajosa al implementar la selección genómica, ya que por lo general no todos los individuos pueden genotiparse y en ocasiones (principalmente en animales) no se tienen valores fenotípicos para caracteres de interés (por ejemplo, la producción de leche en machos) [@cite:17; @cite:31; @cite:11; @cite:21]. Como solución al problema de la falta de fenotipos, @cite:39, con la implementación del GBLUP, propuso asignarle pseudo-fenotipos o valores de-regresados (estos son, valores fenotípicos  estimados a partir de los EBV) a aquellos individuos con fenotipos faltantes, basándose en la información de sus parientes, permitiendo de esta forma implementar la selección genómica combinando los EBV y los genotipos a través de múltiples pasos (Figura 2.7) [@cite:17; @cite:16; @cite:14; @cite:15; @cite:18].
...
data.frame(
  ...
  annotate(geom = 'text', x = 9.4, y = 1.0, label = 'GEBV', size = 4.8, colour = 'black') +
...
### Factores que afectan la precisión de la predicción de la selección genómica

La precisión con la que se predice el GEBV es el factor más importante que determina el éxito de la selección genómica [@cite:6]. Dicha precisión se suele evaluar como la correlación entre el GEBV predicho y el valor fenotípico real u observado, lo que se denomina como precisión de la predicción [@cite:45]. Asimismo, la precisión de la predicción de la selección genómica depende principalmente del número de individuos genotipados y la densidad de marcadores [@cite:45; @cite:7].

Con relación a la densidad de marcadores, hoy en día se conoce que la precisión de la predicción de la selección genómica disminuye a menor número de marcadores de ADN en desequilibrio de ligamiento con el QTL <!-- NOTA: el desequilibrio de ligamiento entre los marcadores y el QTL es el factor clave del exito de la selecció genómica. Como la selección genómica se basa en el desequilibrio de ligamiento entre el marcador de ADN y el QTL, es necesario gran cantidad de marcadores para garantizar que si haya una asociación con el QTL. --> [@cite:10; @cite:45]. Asimismo, los estudios de simulación indican que a mayor número de marcadores se mejora la precisión de la predicción de la selección genómica [@cite:7]. Sin embargo, el aumento de la precisión puede alcanzar su límite a medida que se aumenta la densidad del marcador [@cite:3; @cite:37; @cite:45], por lo cual se recomienda mediante simulación evaluar el efecto del número de marcadores de ADN en la precisión de la predicción, antes de implementar la selección genómica en escenarios prácticos [@cite:47].

En cuanto al número de individuos genotipados, la precisión de la predicción de la selección genómica es mayor a medida que aumenta su proporción. Esto lo evidencian @cite:6, @cite:3, y @cite:10, quienes coinciden en afirmar que a mayor tamaño de la población de entrenamiento, mayor será la precisión de la predicción del GEBV.
...
::: {.center data-latex=""}
__Figura 2.7:__ . Figura adaptada de @cite:7.
:::
...
Tras la domesticación de animales y el cultivo de plantas, la especie humana consiguió producir animales y plantas mejoradas gracias a la selección artificial. La clave del éxito de los antiguos domesticadores y cultivadores consistió en seleccionar y cruzar individuos portadores de caracteres deseables, al comprender que los descendientes podrían heredar estas características, aunque se desconocían los mecanismos biológicos de la herencia [@cite:43]. Hoy en día, se conoce como mejora genética al proceso sistemático de mejorar los caracteres fenotípicos deseables en animales y plantas mediante selección artificial [@cite:7].
...
```
__Figura 2.8:__ Esquema de la mejora genética en animales y en plantas. Figura adaptada de @cite:44. <!-- NOTA: el esquema de la mejora genética en plantas consiste en lo que se conoce como método de pedigrí, donde se cruzan progenitores y se generan poblaciones segregantes, a través de generaciones de autopolinización y selección, hasta obtener un conjunto de líneas derivadas que combina las buenas características de ambos progenitores. El método de pedigrí es atractivo para los mejoradores porque permite construir mejores variedades reuniendo, en la misma planta, buenas características que estaban presentes en diferentes materiales. Como todos los cruces son controlados, es posible conocer la genealogía de cada cultivar. La principal debilidad del método genealógico reside en el hecho de que el rendimiento se evalúa eficientemente sólo al final del proceso, en líneas puras, cuando la semilla está disponible para ensayos repetidos. -->
...
Finalmente, es necesario multiplicar o difundir el mérito genético medio obtenido de la población mejorada, y así facilitar que en las granjas comerciales los agricultores produzcan los caracteres mejorados [@cite:21]. Según @cite:44, una diferencia importante entre la mejora genética de animales y plantas, es que en animales la mejora difundida a las granjas comerciales no se utiliza en los núcleos de selección, mientras que en plantas, los productos mejorados (en forma de variedades mejoradas) obtenidos en cualquier parte de un ciclo de cultivo, si pueden ser usados como progenitores en un nuevo ciclo. <!-- NOTA: Los fitomejoradores tradicionalmente tienden a centrarse en el desarrollo de productos que consiste en identificar variedades, en lugar de en la mejora de la población, que consiste en identificar progenitores para nuevos cruces. En otras palabras, los mejoradores están más interesados en el valor genotípico que comprende la contribución genética completa al fenotipo que en el valor genético aditivo (el valor genético). Un enfoque en este último es natural en la cría lechera, donde el valor de cría del toro se define indirectamente por el desempeño de su descendencia, no por su propio fenotipo ( Mrode, 2014 ).-->

Por otro lado, desde el momento en que la selección genómica fue propuesta por @cite:8, se adapto rápidamente a la mejora genética en animales, principalmente al sector ganadero. Sin embargo, el uso de la selección genómica en plantas no se ha extendido tanto [@cite:46] y son varias las razones de ello:
...
Pese a lo anterior, en la literatura científica [@cite:87; @cite:32_b; @cite:83; @cite:82] se evidencia el uso potencial de la selección genómica para mejorar el mérito genético medio por selección tanto en animales como en plantas. A pesar de sus diferencias, ambas disciplinas requieren de conceptos y herramientas similares de selección genómica.
<!--\hspace*{1em}
...
## La mejora genética en el arroz

### Importancia de la mejora genética en el arroz

El arroz es uno de los principales cultivos básicos del mundo, ya que proporciona aproximadamente el 20 % de la energía alimentaria mundial [@cite:55]. En la figura a continuación (Figura 2.9) se muestra el aumento en la producción de arroz a lo largo de los años en respuesta a una población mundial en crecimiento. De igual modo, en los próximos años se prevé que dicho crecimiento en la población mundial continúe, lo cual requerirá del aumento en la producción de arroz en relación con los niveles actuales [@cite:44; @cite:32_b]. <!--Garantizar la oferta de este alimento debe ser prioritario para cualquier país que entienda que de ello depende la vida de quienes lo habitan.-->
...
```
__Figura 2.9:__ Evolución de la producción y uso del arroz <!--(_Oryza sativa L._)--> a nivel mundial. Se utilizaron las bases de datos estadísticos de la Organización para la Alimentación y la Agricultura (FAOSTAT) para desarrollar estas figuras.


<!--En este sentido, la tercera revolución agrícola o revolución verde tiene como objetivo aumentar la producción mundial de cultivos, como es el cultivo de arroz, mediante el empleo de métodos de mejora genética [@cite:7], lo cual garantizaría la oferta de arroz en un contexto de una población en crecimiento.-->

### Breve descripción de la mejora genética en plantas con ejemplos en el arroz

El método tradicional y más sencillo en la mejora genética en plantas es la selección de individuos de una sola línea <!-- NOTA: una línea pura es una población de individuos en la que todos portan el mismo genotipo completamente homocigótico. Es un grupo de individuos idénticos que producen siempre descendencia del mismo fenotipo cuando se cruzan entre sí. -->de acuerdo a su fenotipo. Este método, conocido como selección masal, se implementó con un conocimiento elemental de la herencia, basado en el supuesto de que los descendientes de los mejores individuos heredarían sus características [@cite:57]. En el arroz, @cite:58 citan distintos estudios donde se muestra la mejora genética lograda en este cultivo a través del método de selección masal.
...
data.frame(
  ...
  annotate(geom = 'rect', xmin = 2.4, xmax = 4.4, ymin = 8.3, ymax = 9.3, colour = 'yellow', fill = 'yellow', alpha = 0.2) +
  annotate(geom = 'text', x = 3.4, y = 9.0, label = 'Línea', family = 'gochi', size = 3.4, colour = 'black') +
  annotate(geom = 'text', x = 3.4, y = 8.6, label = 'donante', family = 'gochi', size = 3.4, colour = 'black') +
  
  annotate(geom = 'text', x = 5.0, y = 8.8, label = 'x', family = 'gochi', size = 4.4, colour = 'black') +
  
  annotate(geom = 'rect', xmin = 5.55, xmax = 7.65, ymin = 8.3, ymax = 9.3, colour = 'yellow', fill = 'yellow', alpha = 0.2) +
  annotate(geom = 'text', x = 6.6, y = 9.0, label = 'Línea', family = 'gochi', size = 3.4, colour = 'black') +
  annotate(geom = 'text', x = 6.6, y = 8.6, label = 'receptora', family = 'gochi', size = 3.4, colour = 'black') +
  
  geom_curve(x = 4.9, xend = 4.4, y = 7.7, yend = 7.4, arrow = arrow(length = unit(0.05, 'inch')), size = 0.1, color = 'black', curvature = 0.0) +
  
  annotate(geom = 'rect', xmin = 3.4, xmax = 4.2, ymin = 6.9, ymax = 7.5, colour = 'yellow', fill = 'yellow', alpha = 0.2) +
  annotate(geom = 'text', x = 3.8, y = 7.2, label = 'F1', family = 'gochi', size = 3.4, colour = 'black') +
  
  annotate(geom = 'text', x = 5.0, y = 7.2, label = 'x', family = 'gochi', size = 4.4, colour = 'black') +
  
  annotate(geom = 'rect', xmin = 5.8, xmax = 6.6, ymin = 6.9, ymax = 7.5, colour = 'yellow', fill = 'yellow', alpha = 0.2) +
  annotate(geom = 'text', x = 6.2, y = 7.2, label = 'SR', family = 'gochi', size = 3.4, colour = 'black') +
  
  geom_curve(x = 4.9, xend = 4.4, y = 6.7, yend = 6.4, arrow = arrow(length = unit(0.05, 'inch')), size = 0.1, color = 'black', curvature = 0.0) +
  geom_curve(x = 6.2, xend = 6.2, y = 6.8, yend = 6.6, arrow = arrow(length = unit(0.05, 'inch')), size = 0.1, color = 'black', curvature = 0.0) +
  
  annotate(geom = 'rect', xmin = 3.35, xmax = 4.25, ymin = 5.9, ymax = 6.5, colour = 'yellow', fill = 'yellow', alpha = 0.2) +
  annotate(geom = 'text', x = 3.8, y = 6.2, label = 'RC1', family = 'gochi', size = 3.4, colour = 'black') +
  
  annotate(geom = 'text', x = 5.0, y = 6.2, label = 'x', family = 'gochi', size = 4.4, colour = 'black') +
  
  annotate(geom = 'rect', xmin = 5.8, xmax = 6.6, ymin = 5.9, ymax = 6.5, colour = 'yellow', fill = 'yellow', alpha = 0.2) +
  annotate(geom = 'text', x = 6.2, y = 6.2, label = 'SR', family = 'gochi', size = 3.4, colour = 'black') +
  
  geom_curve(x = 4.9, xend = 4.4, y = 5.7, yend = 5.4, arrow = arrow(length = unit(0.05, 'inch')), size = 0.1, color = 'black', curvature = 0.0) +
  geom_curve(x = 4.9, xend = 4.4, y = 5.2, yend = 4.9, arrow = arrow(length = unit(0.05, 'inch')), size = 0.1, color = 'black', curvature = 0.0) +
  geom_curve(x = 4.9, xend = 4.4, y = 4.7, yend = 4.4, arrow = arrow(length = unit(0.05, 'inch')), size = 0.1, color = 'black', curvature = 0.0) +
  
  geom_curve(x = 6.2, xend = 6.2, y = 5.7, yend = 5.5, arrow = arrow(length = unit(0.05, 'inch')), size = 0.1, color = 'black', curvature = 0.0) +
  geom_curve(x = 6.2, xend = 6.2, y = 5.3, yend = 5.1, arrow = arrow(length = unit(0.05, 'inch')), size = 0.1, color = 'black', curvature = 0.0) +
  geom_curve(x = 6.2, xend = 6.2, y = 4.9, yend = 4.7, arrow = arrow(length = unit(0.05, 'inch')), size = 0.1, color = 'black', curvature = 0.0) +
  
  annotate(geom = 'rect', xmin = 3.1, xmax = 4.2, ymin = 3.9, ymax = 4.5, colour = 'yellow', fill = 'yellow', alpha = 0.2) +
  annotate(geom = 'text', x = 3.65, y = 4.2, label = 'RCn-1', family = 'gochi', size = 3.4, colour = 'black') +
  
  annotate(geom = 'text', x = 5.0, y = 4.2, label = 'x', family = 'gochi', size = 4.4, colour = 'black') +
  geom_curve(x = 4.9, xend = 4.4, y = 3.7, yend = 3.4, arrow = arrow(length = unit(0.05, 'inch')), size = 0.1, color = 'black', curvature = 0.0) +
  geom_curve(x = 5.1, xend = 5.6, y = 3.7, yend = 3.4, arrow = arrow(length = unit(0.05, 'inch')), size = 0.1, color = 'black', curvature = 0.0) +
  
  annotate(geom = 'rect', xmin = 5.8, xmax = 6.6, ymin = 3.9, ymax = 4.5, colour = 'yellow', fill = 'yellow', alpha = 0.2) +
  annotate(geom = 'text', x = 6.2, y = 4.2, label = 'SR', family = 'gochi', size = 3.4, colour = 'black') +
  
  annotate(geom = 'rect', xmin = 3.1, xmax = 4.2, ymin = 2.9, ymax = 3.5, colour = 'yellow', fill = 'yellow', alpha = 0.2) +
  annotate(geom = 'text', x = 3.65, y = 3.2, label = 'RCn', family = 'gochi', size = 3.4, colour = 'black') +
  
  annotate(geom = 'text', x = 5.0, y = 3.2, label = 'x', family = 'gochi', size = 4.4, colour = 'black') +
  geom_curve(x = 4.9, xend = 4.4, y = 2.3, yend = 2.0, arrow = arrow(length = unit(0.05, 'inch')), size = 0.1, color = 'black', curvature = 0.0) +
  geom_curve(x = 5.1, xend = 5.6, y = 2.3, yend = 2.0, arrow = arrow(length = unit(0.05, 'inch')), size = 0.1, color = 'black', curvature = 0.0) +
  
  annotate(geom = 'rect', xmin = 5.8, xmax = 6.9, ymin = 2.9, ymax = 3.5, colour = 'yellow', fill = 'yellow', alpha = 0.2) +
  annotate(geom = 'text', x = 6.35, y = 3.2, label = 'RCn', family = 'gochi', size = 3.4, colour = 'black') +
  
  annotate(geom = 'rect', xmin = 3.35, xmax = 4.25, ymin = 1.5, ymax = 2.1, colour = 'yellow', fill = 'yellow', alpha = 0.2) +
  annotate(geom = 'text', x = 3.8, y = 1.8, label = 'EC1', family = 'gochi', size = 3.4, colour = 'black') +
  
  annotate(geom = 'text', x = 5.0, y = 1.8, label = 'x', family = 'gochi', size = 4.4, colour = 'black') +
  
  annotate(geom = 'rect', xmin = 5.8, xmax = 6.9, ymin = 1.5, ymax = 2.1, colour = 'yellow', fill = 'yellow', alpha = 0.2) +
  annotate(geom = 'text', x = 6.35, y = 1.8, label = 'EC1', family = 'gochi', size = 3.4, colour = 'black') +
  
  annotate(geom = 'rect', xmin = 3.35, xmax = 4.25, ymin = 0.5, ymax = 1.1, colour = 'yellow', fill = 'yellow', alpha = 0.2) +
  annotate(geom = 'text', x = 3.8, y = 0.8, label = 'EC2', family = 'gochi', size = 3.4, colour = 'black') +
  
  annotate(geom = 'text', x = 5.0, y = 0.8, label = 'x', family = 'gochi', size = 4.4, colour = 'black') +
  geom_curve(x = 4.9, xend = 4.4, y = 1.3, yend = 1.0, arrow = arrow(length = unit(0.05, 'inch')), size = 0.1, color = 'black', curvature = 0.0) +
  geom_curve(x = 5.1, xend = 5.6, y = 1.3, yend = 1.0, arrow = arrow(length = unit(0.05, 'inch')), size = 0.1, color = 'black', curvature = 0.0) +
  
  annotate(geom = 'rect', xmin = 5.8, xmax = 6.9, ymin = 0.5, ymax = 1.1, colour = 'yellow', fill = 'yellow', alpha = 0.2) +
  annotate(geom = 'text', x = 6.35, y = 0.8, label = 'EC2', family = 'gochi', size = 3.4, colour = 'black') +
  
  annotate(geom = 'rect', xmin = 3.3, xmax = 6.7, ymin = -0.9, ymax = -0.3, colour = 'yellow', fill = 'yellow', alpha = 0.2) +
  annotate(geom = 'text', x = 5.0, y = -0.6, label = 'Línea mejorada', family = 'gochi', size = 3.4, colour = 'black') +
  
  geom_curve(x = 5.0, xend = 5.0, y = 0.4, yend = -0.2, arrow = arrow(length = unit(0.05, 'inch')), size = 0.1, color = 'black', curvature = 0.0) +
  
  annotate(geom = 'segment', x = 2.8, xend = 2.8, y = 7.5, yend = 2.9, colour = 'black', linetype = 'solid') +
  annotate(geom = 'text', x = 2.4, y = 5.25, label = 'Retrocruzamiento', size = 3.2, colour = 'black', angle = 90) +
  
  annotate(geom = 'segment', x = 2.8, xend = 2.8, y = 2.1, yend = 0.5, colour = 'black', linetype = 'solid') +
  annotate(geom = 'text', x = 2.4, y = 1.3, label = 'Entrecruzamiento', size = 3.2, colour = 'black', angle = 90) +
  
  annotate(geom = 'segment', x = 7.1, xend = 7.1, y = 7.5, yend = 3.9, colour = 'black', linetype = 'solid') +
  annotate(geom = 'text', x = 7.5, y = 5.7, label = 'Selección recurrente', size = 3.2, colour = 'black', angle = 90) +
  
  scale_x_continuous(limits = c(0.0, 10.0)) +
  scale_y_continuous(limits = c(-1.0, 9.4)) +
  
  theme_bw() +
  #labs(title = 'Apareamiento controlado en plantas') +
  theme(axis.text = element_blank(),
        axis.ticks = element_blank(),
        axis.title = element_blank(),
        plot.title = element_text(size = 11, face = 'bold')) -> Ap_control
...
data.frame(
  ...
  annotate(geom = 'rect', xmin = 1.4, xmax = 2.4, ymin = 9.4, ymax = 9.9, colour = 'black', fill = 'white', alpha = 0.2) +
  annotate(geom = 'text', x = 1.9, y = 9.65, label = 'EH', family = 'gochi', size = 3.4, colour = 'black') +
  annotate(geom = 'rect', xmin = 1.4, xmax = 2.4, ymin = 8.85, ymax = 9.35, colour = 'black', fill = 'white', alpha = 0.2) +
  annotate(geom = 'text', x = 1.9, y = 9.1, label = 'ES', family = 'gochi', size = 3.4, colour = 'black') +
  annotate(geom = 'text', x = 0.6, y = 9.375, label = 'Año 1', size = 3.4, colour = 'black') +
  annotate(geom = 'rect', xmin = 2.8, xmax = 5.3, ymin = 8.85, ymax = 9.9, colour = 'cyan', fill = 'cyan', alpha = 0.2) +
  annotate(geom = 'text', x = 4.05, y = 9.575, label = 'Cruces y vali-', family = 'gochi', size = 3.4, colour = 'black') +
  annotate(geom = 'text', x = 4.05, y = 9.175, label = 'dación de F1', family = 'gochi', size = 3.4, colour = 'black') +
  
  annotate(geom = 'rect', xmin = 1.4, xmax = 2.4, ymin = 8.2, ymax = 8.7, colour = 'black', fill = 'white', alpha = 0.2) +
  annotate(geom = 'text', x = 1.9, y = 8.45, label = 'EH', family = 'gochi', size = 3.4, colour = 'black') +
  annotate(geom = 'rect', xmin = 1.4, xmax = 2.4, ymin = 7.65, ymax = 8.15, colour = 'black', fill = 'white', alpha = 0.2) +
  annotate(geom = 'text', x = 1.9, y = 7.9, label = 'ES', family = 'gochi', size = 3.4, colour = 'black') +
  annotate(geom = 'text', x = 0.6, y = 8.175, label = 'Año 2', size = 3.4, colour = 'black') +
  annotate(geom = 'rect', xmin = 2.8, xmax = 5.3, ymin = 7.65, ymax = 8.7, colour = 'cyan', fill = 'cyan', alpha = 0.2) +
  annotate(geom = 'text', x = 4.05, y = 8.375, label = 'Rápido avance', family = 'gochi', size = 3.4, colour = 'black') +
  annotate(geom = 'text', x = 4.05, y = 7.975, label = 'de generación', family = 'gochi', size = 3.4, colour = 'black') +
  
  scale_x_continuous(limits = c(0.0, 10.0)) +
  scale_y_continuous(limits = c(0.0, 10.0)) +
  
  theme_bw() +
  labs(title = 'Bondades de la selección genómica') +
  theme(axis.text = element_blank(),
        axis.ticks = element_blank(),
        axis.title = element_blank(),
        plot.title = element_text(size = 11, face = 'bold')) -> Ap_GS




#(Ap_control | Ap_control) -> GS_plant

knitr::include_graphics('figures/Ap_control.pdf', auto_pdf = TRUE)
```
__Figura 2.10:__ Esquema de mejora genética de especies agrícolas. Figura adaptada de @cite:59.

En la selección fenotípica recurrente, la mejora genética se realiza a través de un esquema de rondas consecutivas de selección. En esta estrategia de apareamiento, se cruzan individuos progenitores (líneas donante y receptora) y se generan poblaciones segregantes <!-- NOTA: una línea segregante es un grupo de individuos genéticamente variables que constituyen la descendencia de una población híbrida que resulta de cruzar dos líneas diferentes genéticamente uniformes. -->a través de generaciones de autopolinización <!-- NOTA: la polinización directa o autopolinización o autofecundación sucede cuando el grano de polen llega solo desde el estambre al estigma de la misma flor. A estas especies se las denomina autógamas. La ventaja de esta tipología es que, en caso de ausencia de agentes polinizadores externos, la especie puede reproducirse igual. -->y selección, hasta obtener variedades mejoradas que combinan las buenas características y los avances logrados en generaciones anteriores. La característica principal de la selección fenotípica recurrente es aumentar las frecuencias de los alelos favorables [@cite:62].

En cuanto a la introgresión, esta inicia con el cruce de individuos progenitores, seguido de repetidos ciclos de retrocruzamiento y entrecruzamiento<!-- NOTA: el objetivo de un programa de introgresión es introducir un gen 'objetivo', que puede ser un solo gen o un QTL, de una línea o raza (donante) de baja productividad en una línea productiva que carece de ese gen particular. La introgresión comienza cruzando las líneas donante y receptora, seguida de repetidos retrocruzamientos con la línea receptora para recuperar el genoma de la línea receptora. El gen objetivo se mantiene en las generaciones de retrocruzamiento a través de la selección de portadores de genes donantes. La recuperación del genoma del receptor puede mejorarse mediante la selección de individuos retrocruzados que tengan un alto valor para el fenotipo del rasgo del receptor. Una vez que se recupera una proporción suficiente del genoma del receptor, la línea de retrocruzamiento se entrecruza y se seleccionan los homocigotos del gen donante para fijar el gen objetivo. Esto podría requerir más de una generación para obtener suficientes individuos para la reproducción posterior o si se deben introgresar varios genes objetivo. -->. El proposito de esta estrategia de apareamiento consiste en generar variedades mejoradas (híbridas) reuniendo en una misma planta las mejores características de los distintos cruces [@cite:57], aumentando también el rendimiento más allá de las variedades iniciales mediante la explotación de la heterosis [@cite:62]. <!-- NOTA: se espera que los cruces entre líneas genéticamente distantes muestren mayor vigor híbrido o efectos heteróticos que aquellos entre líneas más estrechamente relacionadas, porque se espera que las diferencias en las frecuencias alélicas entre líneas genéticamente distantes sean mayores. -->

En relación con las estrategias descritas en el párrafo anterior, @cite:65 hacen mención del programa de mejora genética en el arroz para América Latina y el Caribe, desarrollados por el centro internacional de agricultura tropical, y por la organización francesa de investigación agrícola y de cooperación internacional para el desarrollo sostenible. Los autores describen a este programa como un proceso de tres etapas realizadas de manera recurrente: <!--(i)--> evaluación de individuos derivados de una población inicial<!-- ($P_{n}$)-->, <!--(ii)--> selección con presión suave de los mejores individuos para aumentar gradualmente la frecuencia de alelos favorables, y <!--(iii)--> entrecruzamiento de los individuos seleccionados para formar una nueva población <!--($P_{n+1}$)--> con una media mejorada. <!--Al final de cada ciclo, los mejores individuos seleccionados también se utilizan para el desarrollo de variedades mejoradas a través del mejoramiento genético convencional.-->

<!--NOTA: Desde 1992, el CIAT (Centro internacional de agricultura tropical) y el Cirad (Organización francesa de investigación agrícola y cooperación internacional para el desarrollo sostenible) han desarrollado un programa de mejoramiento de arroz para América Latina y el Caribe basado en el mejoramiento de poblaciones sintéticas a través de la selección recurrente. El programa comenzó con el desarrollo de una población base P0 a través del entrecruzamiento de aproximadamente 60 accesiones fundadoras complementarias. La población base resultante se recombinó varias veces para generar una gran cantidad de recombinantes, lo que condujo a una población sintética con desequilibrio de ligamiento más bien bajo, dotada de suficiente variabilidad genética para la selección futura. El esquema de selección incluye tres etapas realizadas de manera recurrente: (i) evaluación de individuos derivados de una población Pn , (ii) selección con presión suave de los mejores individuos para aumentar gradualmente la frecuencia de alelos favorables en loci involucrados en caracteres complejos (aditivos y efectos de epistasis), (iii) entrecruzamiento de los individuos seleccionados para formar una nueva población Pn+1 con una media mejorada. Este esquema se ve facilitado por la presencia de un gen de esterilidad masculina nuclear recesivo (ms-IR36) ubicado en el cromosoma 2, que se segrega dentro de la población. Al final de cada ciclo, los mejores individuos seleccionados también se utilizan para el desarrollo de variedades mejoradas a través del mejoramiento genético convencional.-->

La MAS también fue importante en la mejora genética de plantas [@cite:60], ya que permitió identificar e incorporar QTL beneficiosos en tiempos relativamente cortos [@cite:61]. Sin embargo, el impacto general al implementar este método para aumentar la eficiencia de la mejora genética en el arroz ha sido limitada [@cite:63], puesto que la mayoría de caracteres de interés en este cultivo no están controlados por pocos genes con grandes efectos (siendo esta la principal característica para que la MAS sea efectiva), sino por muchos genes de pequeño efecto y/o por una combinación de genes mayores y menores [@cite:32; @cite:60].

La selección genómica brindó nuevas oportunidades para la mejora genética de caracteres con herencia poligénica [@cite:64]. Este método se implementó para la mejora de la ganancia genética en plantas, debido al éxito alcanzado en animales [@cite:60]<!-- NOTA: el uso de la selección genómica en esquemas de fitomejoramiento comenzó con investigaciones sobre la precisión de las predicciones de los GEBV basadas en estudios de simulación -->, y hoy en día se ha establecido como un método prometedor [@cite:63]. En el arroz, los primeros estudios que informaron del uso de la selección genómica fueron publicados en el año 2014, siendo esto considerado por @cite:58 como un hecho donde se evidencia un retraso en la transición del mapeo de QTL a la selección genómica, debido a que años atrás ya se contaba con los recursos genómicos necesarios para su implementación.

En plantas, una de las ventajas con la implementación de la selección genómica es que permite reducir el tiempo requerido para el desarrollo de variedades mejoradas. @cite:64 aluden esta ventaja en la mejora genética del tomate, donde a partir del uso de la selección genómica es posible reducir el número de generaciones de autopolinización necesarias para generar variedades de tomate de buen rendimiento. De igual modo, @cite:37 mencionan que el uso de la selección genómica en la mejora genética del maíz conlleva una reducción de los costos de mejoramiento comparado con el método tradicional de selección fenotípica, al reducir el tiempo requerido para el desarrollo de variedades. En cuanto a la mejora genética del arroz, @cite:58 hacen mención que el uso de la selección genómica permite aumentar la tasa de ganancia genética en este cultivo al reducir el tiempo necesario en la generación de variedades mejoradas.

La mayor parte de los estudios sobre selección genómica en plantas han ignorado la información de pedigrí, a diferencia de los animales donde el uso del pedigrí en modelos de selección ha sido considerado un factor importante en los programas de mejora genética [@cite:63]. <!-- Por lo tanto, no se dispone de una comprensión integral de las ganancias de la selección fenotípica a la selección de pedigrí en las plantas. -->Con el desarrollo del ssGBLUP existe la posibilidad de usar esta información, y hoy en día se pueden encontrar ejemplos del uso de esta metodología en plantas [@cite:19; @cite:11]. Sin embargo, todavía se necesitan esquemas de selección genómica optimizados y validados en el arroz. En estos momentos, no se han evaluado los distintos factores que pueden afectar la implementación y precisión del método del ssGBLUP en cultivos de arroz. Contar con estos resultados, ya sea a través de enfoques de simulación o usando datos empíricos, será clave para evaluar el impacto de dichos factores al implementar el ssGBLUP en el arroz.
