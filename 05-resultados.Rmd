...
## Efecto de la densidad del marcador y de la cantidad de individuos genotipados en la precisión de la predicción mediante simulaciones hacia atrás
...
::: {.center data-latex=""}
__Figura 5.2:__ Precisión de la predicción con tamaños crecientes en el número de marcadores y de individuos genotipados. Los tres pedigríes (Pedigrí 1-2-3) corresponden a los pedigríes generados en simulaciones hacia atras.
:::

## Efecto de la densidad del marcador y de la cantidad de individuos genotipados en la precisión de la predicción mediante simulaciones hacia adelante
...
```{r Graficas de mapa de calor e histograma de las matrices de relaciones (A y H), echo = FALSE, out.width = "100%", fig.align = 'center', message = FALSE}
...
mH_ped4a_map_a <- mH_ped4a_map %>%
  ...
  geom_tile() +
  scale_fill_gradient(high = 'red', low = 'yellow') +
  labs(title = 'Matriz H (F0-F1-F2-F3)', x = 'Individuos', y = 'Individuos', fill = 'Relación') +
  theme_bw() +
  theme(
    axis.text = element_text(size = 10, face = 'bold'),
    axis.title = element_text(size = 10, face = 'bold'),
...
mH_ped4b_map_a <- mH_ped4b_map %>%
  ...
  geom_tile() +
  scale_fill_gradient(high = 'red', low = 'yellow') +
  labs(title = 'Matriz H (F1-F2)', x = 'Individuos', y = 'Individuos', fill = 'Relación') +
  theme_bw() +
  theme(
    axis.text = element_text(size = 10, face = 'bold'),
    axis.title = element_text(size = 10, face = 'bold'),
...
mH_ped4c_map_a <- mH_ped4c_map %>%
  ...
  labs(title = 'Matriz H (F1)', x = 'Individuos', y = 'Individuos', fill = 'Relación') +
  theme_bw() +
  theme(
    axis.text = element_text(size = 10, face = 'bold'),
    axis.title = element_text(size = 10, face = 'bold'),
...
mH_ped4c_map_b <- ggplot(data = mH_ped4c_map, aes(x = value, after_stat(density))) +
  geom_histogram(colour = 'cyan', fill = 'cyan', alpha = 0.2) +
  labs(title = 'Matriz H (F1)', x = 'relación', y = ' ') +
  theme_bw() +
  theme(
    axis.text.x = element_text(size = 10, face = 'bold'),
    axis.title.x = element_text(size = 10, face = 'bold'),
...
(mA_ped4_map_b | mH_ped4c_map_b ) / (mH_ped4b_map_b | mH_ped4a_map_b) -> dens_rel

(mA_ped4_map_a | mH_ped4c_map_a) / (mH_ped4b_map_a | mH_ped4a_map_a) -> map_rel

knitr::include_graphics('figures/dens_rel.pdf', auto_pdf = TRUE)
knitr::include_graphics('figures/map_rel.pdf', auto_pdf = TRUE)
```

```{r Análisis de componentes principales para las matrices de relaciones combinadas (H), echo = FALSE, out.width = "100%", fig.align = 'center', message = FALSE}
