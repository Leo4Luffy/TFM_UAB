...
pacman::p_load(cowplot, magick, tibble, gt, dplyr, ggplot2, showtext, here, readr, latex2exp)
...
::: {.center data-latex=""}
__Tabla 5.1:__ Estimaciones de heredabilidad para el caracter tiempo de floración mediante el método BLUP.
:::
...
knitr::include_graphics('figures/covar_h2_2.pdf', auto_pdf = TRUE)
```

::: {.center data-latex=""}
__Figura 5.1:__ Distribuciones posteriores de heredabilidad obtenidas mediante enfoques Bayesianos. Los valores presentados en la Tabla 5.1 (para el enfoque Bayesiano), corresponden a la media calculada a partir de estas distribuciones.
:::
...
mA_ped4_map <- melt(mA_ped4) # Usando la función melt() del paquete "reshape2" se convertie la matriz A al formato largo.

mA_ped4_map_a <- ggplot(data = mA_ped4_map, aes(x = Var1, y = Var2, fill = value)) +
  geom_tile() +
  scale_fill_gradient(high = 'red', low = 'yellow') +
  labs(x = 'Individuos', y = 'Individuos', fill = 'Relación') +
  theme_bw() +
...
mA_ped4_map_b <-ggplot(data = mA_ped4_map, aes(x = value, after_stat(density))) +
  geom_histogram(colour = 'cyan', fill = 'cyan', alpha = 0.2) +
  labs(title = 'Pedigrí 4 - matriz A', x = 'relación', y = ' ') +
  theme_bw() +
  theme(
    axis.text.x = element_text(size = 10, face = 'bold'),
    axis.title.x = element_text(size = 10, face = 'bold'),
...
mH_ped4 <- readRDS(here('figures', 'mH_ped4.RDS'))

mH_ped4_map <- melt(mH_ped4) # Usando la función melt() del paquete "reshape2" se convertie la matriz H al formato largo.

mH_ped4_map_a <- ggplot(data = mH_ped4_map, aes(x = Var1, y = Var2, fill = value)) +
  geom_tile() +
  scale_fill_gradient(high = 'red', low = 'yellow') +
  labs(x = 'Individuos', y = 'Individuos', fill = 'Relación') +
  theme_bw() +
...
mH_ped4_map_b <-ggplot(data = mH_ped4_map, aes(x = value, after_stat(density))) +
  geom_histogram(colour = 'cyan', fill = 'cyan', alpha = 0.2) +
  labs(title = 'Pedigrí 4 - matriz H', x = 'relación', y = ' ') +
  theme_bw() +
  theme(
    axis.text.x = element_text(size = 10, face = 'bold'),
    axis.title.x = element_text(size = 10, face = 'bold'),
...
(mA_ped3_map_b | mA_ped3_map_a | mH_ped3_map_b | mH_ped3_map_a) / # Pedigrí 1
  (mA_ped2_map_b | mA_ped2_map_a | mH_ped2_map_b | mH_ped2_map_a) / # Pedigrí 2
  (mA_ped1_map_b | mA_ped1_map_a | mH_ped1_map_b | mH_ped1_map_a) / # Pedigrí 3
  (mA_ped4_map_b | mA_ped4_map_a | mH_ped4_map_b | mH_ped4_map_a) # Pedigrí 3
